# ADR-002: Separation of Agent Core and Agent Framework
...
* **Single SDK Package**: Combine contracts and executors into a single `@leadforge/agent-sdk`.
  * *Tradeoffs*: Simplifies directory structure, but forces the SDK to be rebuilt whenever we change execution loop structures or upgrade third-party adapters.
* **Direct Orchestration**: Let business agents import Mastra or LangGraph directly.
  * *Tradeoffs*: Eliminates the adapter layer, but couples agents to a single framework's APIs.
