...
/**
 * LeadIntelligenceDetails — displays AI-generated buying intent and specification details.
 *
 * Design updates:
 *   - Squared corners: all cards, badges, and progress containers use rounded-none.
 *   - Design System Colors: matches primary/warning/info/danger/success tokens, no raw roses/ambers/violets.
 */
export default function LeadIntelligenceDetails({ companyId }: LeadIntelligenceDetailsProps) {
  ...
  if (!intel || !intel.opportunityScore) {
    return (
      <div className="bg-card border border-border-subtle rounded-none p-5 space-y-4 text-center font-sans">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-1 max-w-xs mx-auto">
            Extract technical signals, buying intent, decision-maker hierarchy, and AI opening lines
            for this lead.
          </p>
        </div>
  ...
  const scoreColor =
    score.overallScore >= 75
      ...
      : 'text-info bg-info-muted border-info/20';

  const scoreBadge =
    score.overallScore >= 75 ? 'Hot Lead' : score.overallScore >= 45 ? 'Warm Lead' : 'Cold Lead';
  ...
  return (
    <div className="space-y-6 font-sans select-none">
      ...
      <div className="bg-card border border-border-subtle rounded-none p-5 relative overflow-hidden">
        ...
        {/* Explainable Why Log */}
        {score.explanation && (
          <div className="mt-4 bg-surface-3 border border-border-subtle rounded-none p-3 space-y-1.5">
      ...
      <div className="bg-card border border-border-subtle rounded-none p-5 space-y-4">
        ...
        <div className="space-y-2 pt-1 border-t border-border-subtle/40">
          ...
          <div className="bg-surface-3 border border-border-subtle rounded-none p-3 text-[11px] italic text-foreground leading-relaxed">
          ...
          <div className="space-y-1.5 pt-1 border-t border-border-subtle/40">
            ...
            <div className="flex flex-wrap gap-1.5 mt-1">
              {comp.techStack.map((tech: string, i: number) => (
                <span
                  key={i}
                  className="px-2 py-0.5 rounded-none bg-surface-3 border border-border-subtle text-[10px] font-medium text-foreground"
                >
                  {tech}
                </span>
              ))}
