...
export default function SettingsScreen({ integrations, onToggleIntegration, darkMode, onToggleDarkMode }: SettingsProps) {
  return (
    <div className="space-y-6 animate-in fade-in duration-200 text-xs">
      <div>
        <h2 className="text-sm font-semibold tracking-tight text-foreground">Workspace Settings</h2>
        <p className="text-[11px] text-secondary mt-0.5">Integrate third-party platform databases and modify appearances.</p>
      </div>

      {/* Integrations panel */}
      <div className="space-y-4">
        <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted">Active Integrations</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {integrations.map((item) => (
            <div key={item.id} className="bg-card border border-border-subtle rounded-lg p-4 flex items-center justify-between gap-4 hover:border-border-default transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-secondary mt-0.5 truncate max-w-sm">{item.description}</p>
                </div>
              </div>
      ...
      <div className="border-t border-border-subtle pt-6 space-y-4">
        ...
        <div className="bg-card border border-border-subtle rounded-lg p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-secondary mt-0.5">Adapt screen layouts to low light levels.</p>
          </div>
          <button 
