...
export default function WorkflowsScreen() {
  const workflows = [
    { name: 'Lead Acquisition & Verification', active: true, steps: ['Enrich contact via scraper engines', 'Verify email deliverability status', 'Send Slack notification alert', 'Sync with CRM integration'] },
    { name: 'Trigger Campaign outreach on Sign Up', active: false, steps: ['Wait for sign up trigger', 'Add lead into Q3 Campaign list', 'Wait 1 day', 'Send Intro sequence email'] }
  ]
    ...
    <div className="space-y-6 animate-in fade-in duration-200 text-xs">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[11px] text-secondary mt-0.5">Automate leads qualification, scraping, and follow-ups sequences.</p>
        </div>
        ...
        {workflows.map((wf, idx) => (
          <div key={idx} className="p-5 space-y-4">
            <div className="flex justify-between items-start">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${wf.active ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-sunken text-muted'}`}>
