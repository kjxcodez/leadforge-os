...
export async function runObservabilityTests(workspaceId: string): Promise<boolean> {
  ...
  try {
    ...
    const row = db.prepare("SELECT avg(durationMs) as avgVal FROM jobs WHERE type = 'scraper:maps' AND status = 'completed'").get() as any;
  ...
  try {
    ...
    db.prepare("UPDATE jobs SET status = 'queued', retryCount = 0, error = NULL WHERE id = ?").run(failedJobId);
    
    const retriedJob = db.prepare("SELECT status, error FROM jobs WHERE id = ?").get(failedJobId) as any;
