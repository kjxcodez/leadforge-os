 ...
 */
export function resolveVariablesRecursive(val: any, ctx: ExecutionContext, db?: Database.Database, workspaceId?: string): any {
    ...
    if (db && workspaceId && val.includes('{{secret.')) {
      return val.replace(/\{\{secret\.([^}]+)\}\}/g, (_m, key: string) => {
        ...
        for (const pk of possibleKeys) {
          const row = db.prepare('SELECT value FROM settings WHERE workspaceId = ? AND key = ?').get(workspaceId, pk) as { value: string } | undefined;
  ...
  if (Array.isArray(val)) {
    return val.map(item => resolveVariablesRecursive(item, ctx, db, workspaceId));
  }
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (step.type === 'HTTP_REQUEST' && step.config?.saveResponseAs) {
      ...
      if (seenResponseVars.has(saveResponseAs)) {
        errors.push(`Step at index ${i} (HTTP_REQUEST): Duplicate saveResponseAs variable "${saveResponseAs}" across workflow`);
      }
          ...
          if (val.includes('{{secret.')) {
            val.replace(/\{\{secret\.([^}]+)\}\}/g, (_m, key: string) => {
              ...
              if (!/^[a-zA-Z0-9_]+$/.test(trimmed)) {
                errors.push(`Step at index ${i} (${step.type}): Invalid secret reference format in "${path}": "${trimmed}" (must be alphanumeric)`);
              }
              return '';
            });
          }
          if (step.type === 'SET_VARIABLE' && step.config?.assignments) {
            step.config.assignments.forEach((assignment: any) => {
              if (assignment.variable && val.includes(`{{variables.${assignment.variable}}}`)) {
                errors.push(`Step at index ${i} (${step.type}): Recursive variable reference detected: "${assignment.variable}" references itself`);
              }
            });
          }
      ...
      try {
        const stepPromise = (async (): Promise<StepResult> => {
          return await registryAction.execute(
            db, entityId!, ctx.workspaceId, sequenceId!, step, ctx, execCtx!, labelMap
          );
        })();
...
function redactHeaders(headers: Record<string, any>): Record<string, string> {
  ...
  const sensitiveKeys = ['authorization', 'cookie', 'api-key', 'apikey', 'secret', 'password', 'token', 'x-api-key', 'pass', 'sec'];
...
function redactBody(body: any): any {
  ...
  if (typeof body === 'string') {
    try {
      ...
      return body.replace(/(password|token|secret|auth|apikey|api_key|pass|sec)=[^&]*/gi, '$1=[REDACTED]');
    }
  }
...
export function isRetryableHttpError(err: any): boolean {
  ...
  const retryableCodes = ['ENOTFOUND', 'ETIMEDOUT', 'ECONNREFUSED', 'ECONNRESET', 'EADDRINUSE', 'EPIPE'];
...
async function handleHttpRequestStep(
  ...
  const resolvedUrl = String(resolveVariablesRecursive(rawUrl, execCtx, db, workspaceId));
  const redactedHdrs = redactHeaders(resolveVariablesRecursive(step.config?.headers || {}, execCtx, db, workspaceId));
  const resolvedBody = resolveVariablesRecursive(step.config?.body, execCtx, db, workspaceId);

  // Unredacted configurations for sending
  const actualHeaders = resolveVariablesRecursive(step.config?.headers || {}, execCtx, db, workspaceId);
...
export const ActionRegistry: Record<string, AutomationAction> = {
  ...
  ASSIGN_TAG: {
    execute: (db, entityId, workspaceId, _sequenceId, step, ctx, execCtx) => handleAssignTagStep(db, entityId, workspaceId, step, ctx, execCtx),
    validate: (step) => {
      const errors: string[] = [];
  ...
  UPDATE_STAGE: {
    execute: (db, entityId, workspaceId, _sequenceId, step, ctx) => handleUpdateStageStep(db, entityId, workspaceId, step, ctx),
    validate: (step) => {
      const errors: string[] = [];
      const stage = step.config?.stage || step.config?.status;
      if (!stage) {
  ...
  SET_VARIABLE: {
    execute: (_db, _entityId, _workspaceId, _sequenceId, step, _ctx, execCtx) => handleSetVariableStep(step, execCtx),
    validate: (step) => {
      const errors: string[] = [];
  ...
  IF: {
    execute: (_db, _entityId, _workspaceId, _sequenceId, step, ctx, execCtx, labelMap) => handleIfStep(step, execCtx, labelMap, ctx),
  ...
  LABEL: {
    execute: (_db, _entityId, _workspaceId, _sequenceId, step, _ctx, execCtx) => handleLabelStep(step, execCtx),
  ...
  GOTO: {
    execute: (_db, _entityId, _workspaceId, _sequenceId, step, ctx, execCtx, labelMap) => handleGotoStep(step, labelMap, execCtx, ctx),
    validate: (step, labelMap) => {
      const errors: string[] = [];
      const label = step.config?.label;
      if (!label) {
