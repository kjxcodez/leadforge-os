...
export function registerOnboardingIpc() {
  ...
  safeRegister('onboarding:generate-sample-data', async (_event, { workspaceId }) => {
    ...
    const sampleCompanies = [
      ...
      { id: 'sc-02', name: 'Apex Growth Marketing', domain: 'apexgrowth.agency', industry: 'Marketing', status: 'NEW', location: 'New York, NY' },
      { id: 'sc-03', name: 'Summit Retailers', domain: 'summitretail.com', industry: 'Retail', status: 'CUSTOMER', location: 'Austin, TX' }
    ];

    const sampleContacts = [
      { id: 'sct-01', companyId: 'sc-01', firstName: 'Sarah', lastName: 'Connor', email: 'sarah@acmesaas.com', title: 'CEO & Founder', status: 'NEW' },
      { id: 'sct-02', companyId: 'sc-01', firstName: 'John', lastName: 'Doe', email: 'john.doe@acmesaas.com', title: 'VP of Marketing', status: 'NEW' },
      { id: 'sct-03', companyId: 'sc-02', firstName: 'David', lastName: 'Miller', email: 'david@apexgrowth.agency', title: 'Marketing Director', status: 'NEW' },
      { id: 'sct-04', companyId: 'sc-03', firstName: 'Emily', lastName: 'Watson', email: 'emily@summitretail.com', title: 'Operations Lead', status: 'NEW' }
    ];
    ...
    const sampleCampaign = {
      ...
      status: 'Paused',
      description: 'A fully initialized campaign loaded with pre-configured templates and analytics.',
      sequenceId: 'sseq-01',
    ...
    const sampleSequence = {
      ...
      stepsJson: JSON.stringify([
        { name: 'Initial Introduction', delayDays: 0, subject: 'Collaboration Idea', body: 'Hi {{firstName}}, noticed you guys are building at {{companyName}}...' },
        { name: 'Value Proposition Followup', delayDays: 3, subject: 'Quick question regarding your growth tech stack', body: 'Hey {{firstName}}, just following up on our conversation...' }
      ])
    };
    ...
    const transaction = db.transaction(() => {
      ...
      for (const ct of sampleContacts) {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
        `).run(ct.id, workspaceId, ct.companyId, ct.firstName, ct.lastName, ct.email, ct.title, ct.status);
      }
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
      `).run(
        sampleAccount.id, workspaceId, sampleAccount.email, sampleAccount.provider,
        sampleAccount.smtpHost, sampleAccount.smtpPort, sampleAccount.smtpSecure,
        sampleAccount.smtpUsername, sampleAccount.smtpPassword,
        sampleAccount.imapHost, sampleAccount.imapPort, sampleAccount.imapSecure,
        sampleAccount.imapUsername, sampleAccount.imapPassword, sampleAccount.status
      );
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
      `).run(
        sampleCampaign.id, workspaceId, sampleCampaign.name, sampleCampaign.status,
        sampleCampaign.description, sampleCampaign.sequenceId, sampleCampaign.sendingAccountId,
        sampleCampaign.schedule, sampleCampaign.timezone, sampleCampaign.dailyLimit
      );
  ...
  safeRegister('onboarding:save-setting', async (_event, { workspaceId, key, value }) => {
    ...
    const encryptedValue = (key === 'openrouter_key' || key.includes('password')) 
      ? encryptSecret(value)
      : value;
