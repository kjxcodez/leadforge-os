...
import { UserModel, WorkspaceModel, CompanyModel, ContactModel, CampaignModel, OutreachModel } from "../db/index.js";
import { CompanyService, ContactService, CampaignService, WorkspaceService, AuthService, OutreachService } from "../services/index.js";
...
async function run() {
  ...
  console.log("🛡️ Verifying workspace isolation (retrieving Workspace 1 company from Workspace 2 context)...");
  try {
  ...
  console.log("✅ PASS: Duplicate email allowed in different workspace:", contact2.firstName, contact2.email);
  ...
  console.log("✅ PASS: Company still exists in DB with deletedAt timestamp:", deletedDoc.deletedAt);
  ...
  if (paginated.data.length !== 2 || paginated.total < 3) {
    throw new Error(`❌ FAIL: Pagination results invalid (length: ${paginated.data.length}, total: ${paginated.total})`);
  }
