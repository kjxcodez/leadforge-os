# ADR-006: Safety Model & Human Approval Gates
...
* **Full Agent Autonomy**: Let the agent run with zero human gates.
  * *Tradeoffs*: Maximizes execution speed, but increases the risk of spam policy violations, IP blocks, or corrupted database states.
* **Bespoke Agent Validation**: Let each agent implement its own safety checks.
  * *Tradeoffs*: High risk of agents bypassing checks due to prompt injection or coding errors.
